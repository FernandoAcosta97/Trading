...
class TablaCampanas{
	...
	public function mostrarTabla(){
		...
		if($usuario["perfil"] == "admin"){
			...
			$campanas = ControladorCampanas::ctrMostrarCampanasxEstadoAll($item, $valor,"estado",1);
		}
		...
		foreach ($campanas as $key => $value) {
			...
			if($usuario["perfil"] != "admin"){
				...
				if($value["estado"]==1){
					...
					$acciones = "<div class='btn-group'><button type='button' class='btn btn-primary btn-xs btnInvertir' idCampana='".$value["id"]."' data-toggle='modal' data-target='#modalRegistrarComprobante'>Cargar</button></div>";
			...
			if($value["estado"]==2){
				...
				$acciones = "<div class='btn-group'><button class='btn btn-warning btn-xs'><i class='fa fa-pen' style='color:white'></i></button><button class='btn btn-danger btn-xs btnEliminarCampana' idCampana='".$value["id"]."'><i class='fa fa-times' style='color:white'></i></button></div>";
